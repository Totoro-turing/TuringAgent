...
def github_push_node(state: EDWState):
    ...
    # 🎯 发送节点开始进度
    send_node_message(state, "github_push", "started", "开始推送代码到GitHub...")
    ...
    try:
        ...
        # 🎯 发送验证进度
        send_node_message(state, "github_push", "processing", "验证推送参数...", 0.1)

        # 验证必要信息
        if not enhanced_code:
            error_msg = "缺少增强后的代码，无法推送到GitHub"
            logger.error(error_msg)
            send_node_message(state, "github_push", "skipped", "跳过: 缺少增强代码", 1.0)
            return {
        ...
        if not code_path:
            ...
            logger.error(error_msg)
            send_node_message(state, "github_push", "skipped", "跳过: 缺少代码路径", 1.0)
            return {
                "messages": [AIMessage(content=f"GitHub推送跳过: {error_msg}")],
                "user_id": user_id,
                "status": "skipped",
        ...
        if not jira_number:
            logger.warning(f"⚠️ 未找到JIRA号，将使用默认commit消息格式推送到GitHub: {code_path}")
        else:
            logger.info(f"📝 使用JIRA号 {jira_number} 推送代码到GitHub: {code_path}")
        
        logger.info(f"准备将增强后的代码推送到GitHub: {code_path}")
        
        # 🎯 发送初始化进度
        send_node_message(state, "github_push", "processing", "我需要调用GitHub工具...", 0.3)
        
        # 初始化GitHub工具
        try:
            github_tool = GitHubTool(branch = state.get("branch_name"))
        except Exception as e:
            error_msg = f"初始化GitHub工具失败: {str(e)}"
            logger.error(error_msg)
            send_node_message(state, "github_push", "failed", f"错误: {error_msg}", 0.0)
            return {
        ...
        # 🎯 发送推送进度
        send_node_message(state, "github_push", "processing", f"正在推送代码到GitHub: {table_name}", 0.7)
        
        # 推送代码到GitHub
        try:
            # 🎯 构建动态commit消息：JIRA号 + " AiGenerate"
            jira_number = state.get("jira_number", "").strip()
            if jira_number:
                commit_message = f"{jira_number} AiGenerate"
            else:
                # 如果没有JIRA号，使用默认格式
                commit_message = "AiGenerate"
                logger.warning("未找到JIRA号，使用默认commit消息格式")
            ...
            if result.get("status") == "success":
                ...
                message_content += f"Commit消息: {commit_message}\n"
                ...
                # 🎯 发送跳过进度
                send_node_message(state, "github_push", "skipped", "跳过: 代码无变化", 1.0)
                return {
