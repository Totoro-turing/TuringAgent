...
{input}""",
            ...
            "code_enhance_prompt": """你是一个Databricks代码增强专家，负责为数据模型添加新字段。你拥有以下工具：
...
任务流程：
1. 首先使用sql_query工具查询目标表的结构信息：DESCRIBE {table_name} 或 SHOW CREATE TABLE {table_name}
2. 使用code_analysis工具分析源代码，提取所有引用的底表名称
3. 对每个底表使用sql_query工具查询表结构，了解字段类型以便推断新增字段的数据类型
4. 根据收集的信息生成增强后的代码和SQL语句，注意：
   - 如果源代码是{code_type_desc}，增强后的代码也必须是{code_type_desc}
   - 保持与原代码相同的语言类型和编程风格
...
最终请严格按照以下JSON格式返回结果（不要添加任何其他文本）：
{{
  "enhanced_code": "增强后的{code_type_desc}代码，保持与源代码相同的语言类型",
  "new_table_ddl": "包含新字段的完整CREATE TABLE语句",
  "alter_statements": "ALTER TABLE语句，用于添加新字段"
}}""",
